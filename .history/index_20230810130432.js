...
generatebtn.addEventListener('click',()=>{
    ...
    const remainingLength = passwordength - checkcount;
    for (let i = 0; i < remainingLength; i++) {
        // Choose a random character type based on selected checkboxes
        const charType = Math.floor(Math.random() * checkcount);
        if (uppercase.checked && charType === 0) {
            password += generateuppercase();
        } else if (lowercase.checked && charType === 1) {
            password += generatelowercase();
        } else if (numbers.checked && charType === 2) {
            password += generaterandomnumber();
        } else if (symbols.checked && charType === 3) {
            password += generatesymbol();
        }
    }
